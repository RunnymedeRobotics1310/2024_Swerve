...
class CanCoder {
    ...
    CanCoder(int canId, double absoluteEncoderOffset, boolean inverted) {
        ...
        cfg.apply(magnetSensorConfiguration
                ...
                .withSensorDirection(inverted ? SensorDirectionValue.Clockwise_Positive : SensorDirectionValue.CounterClockwise_Positive)
        );

        // note, we aren't bothering to push this offset up into the encoder, we will just manage it here in this class
    ...
    double getAbsolutePositionInDegrees() {
        ...
        if (angle.getStatus() != StatusCode.OK) {
            readingError = true;
            DriverStation.reportWarning("CANCoder " + encoder.getDeviceID() + " reading was faulty after retrying " + maximumRetries + " times. " + angle.getStatus().getDescription(), false);
            return 0;
        }
        if (retryCount > 0) {
            DriverStation.reportWarning("CANCoder " + encoder.getDeviceID() + " read successfully but required " + retryCount + " retries to get valid data.", false);
        }
